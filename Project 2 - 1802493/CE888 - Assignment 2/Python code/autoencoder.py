...
def create_autoencoder(dims):
    ...
    encoded = Dense(dims[4], name='enc_3')(encoded) ##, activation='softmax'
    ...
    decoded = Dense(dims[0], activation='sigmoid', name='dec_0')(decoded) # 
...
# STEP 2: Running DEEP K-Means ------------------------------------------------------------------------------------------------------------------
    ...
    if os.path.exists('./results/AE_results.txt'):
        os.remove('./results/AE_results.txt') # Removes the text file if exists
    with open('./results/AE_results.txt', 'x') as f: 
        ...
        save_confusion_matrix(target, dict_clusterassign[max_acc], specs.n_clusters, 'AE_CM') 
